...
import { useUpdateVechile } from "../hooks/vechile";
...
interface UpdateVechileProps {
  onClose: () => void;
  selectedVechile: Partial<IFormInput>;
}
...
const UpdateVechile = ({ onClose, selectedVechile }: UpdateVechileProps) => {
  const { updateVechiles } = useUpdateVechile();
  const {
    handleSubmit,
    control,
    reset,
    ...
    defaultValues: {
      name: selectedVechile.name,
      status: selectedVechile.status as
        | "Active"
        | "Inactive"
        | "In Maintenance"
        | undefined,
      licensePlate: selectedVechile.licensePlate,
      vehicleModel: selectedVechile.vehicleModel,
      make: selectedVechile.make,
      year: selectedVechile.year,
      mileage: selectedVechile.mileage,
      fuelType: selectedVechile.fuelType as
        | "Petrol"
        | "Diesel"
        | "Electric"
        | "Hybrid"
  ...
  const onSubmit: SubmitHandler<VehicleFormData> = async (data) => {
    try {
      await updateVechiles({ id: selectedVechile._id as string, data });
      reset();
      onClose();
    } catch (error) {
